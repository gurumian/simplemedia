...
int MediaPlayer::Prepare(OnPrepared on_prepared) {
  ...
  source_->PrepareAsync([&](const AVFormatContext *fmt)->int {
    if(source_->HasVideo()) {
      ...
      video_decoder_->Prepare(CodecParam(strm));
    ...
    if(source_->HasAudio()) {
      ...
      audio_decoder_->Prepare(CodecParam(strm));
      ...
      if(audio_renderer_) {
        gurum::AudioSettings in_settings{
    ...
    if(source_->HasSubtitle()) {
      ...
      subtitle_decoder_->Prepare(CodecParam(strm));
