...
module.exports = class MediaPlayer {
  ...
  _ondecodedAudio(frame) {
    ...
    if(frame) {
      ...
      if(video.decoder && !video.decoder.done) return;
  ...
  _ondecodedVideo(frame) {
    ...
    if(frame) {
      ...
      if(audio.decoder && !audio.decoder.done) return;
      if(this.onend) this.onend();
    }
  }

  _ondecode({decoder: decoder, frame: frame}) {
  ...
  _decode(decoder) {
    let done = false;
    while(!done) {
      done = decoder.decode(frame => this._ondecode({
        decoder: decoder,
        frame: frame,
      }));
    }
